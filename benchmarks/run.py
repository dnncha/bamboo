...
from benchmarks.tasks import TASK_SPECS, available_backends
...
def ensure_bench_data(data_dir: Path, record_count: int) -> Path:
    ...
    print(f"Generating benchmark BAM ({record_count:,} records)...")
...
def main() -> int:
    ...
    if args.tasks:
        ...
        task_specs = [(name, fn) for name, fn in TASK_SPECS if name in wanted]
        if not task_specs:
            print(f"No matching tasks for: {', '.join(sorted(wanted))}", file=sys.stderr)
            return 1

    bam_path = ensure_bench_data(args.data_dir, args.records)
    ...
    print(f"Benchmark file: {bam_path}")
    print(f"Backends: {', '.join(backends)}")
    print(f"Tasks: {', '.join(name for name, _ in task_specs)}")
    print()
